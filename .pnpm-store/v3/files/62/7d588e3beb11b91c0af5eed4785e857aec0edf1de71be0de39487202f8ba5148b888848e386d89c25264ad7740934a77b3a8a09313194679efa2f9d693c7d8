...
function openApiMetadataFactory({ schema, io }) {
	...
	const zodId = "_zod" in schema ? zod_v4_core.globalRegistry.get(schema)?.id : void 0;
...
function generateJsonSchema(schema, io) {
	const generatedJsonSchema = "_zod" in schema ? (0, zod_v4_core.toJSONSchema)(schema, { io }) : zodV3ToOpenAPI(schema);
...
Object.defineProperty(exports, "PARENT_METADATA_KEY", {
	enumerable: true,
	get: function() {
		return PARENT_METADATA_KEY;
	}
});
...
Object.defineProperty(exports, "USES_THREE_POINT_ONE_SYNTAX_KEY", {
	enumerable: true,
	get: function() {
		return USES_THREE_POINT_ONE_SYNTAX_KEY;
	}
});
