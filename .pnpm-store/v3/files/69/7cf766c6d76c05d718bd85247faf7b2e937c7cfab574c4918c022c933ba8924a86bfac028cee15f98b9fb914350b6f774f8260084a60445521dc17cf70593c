import { a as convertToOpenApi3Point0, c as EMPTY_TYPE_KEY, d as PARENT_ID_KEY, f as PARENT_METADATA_KEY, g as assert, h as USES_THREE_POINT_ONE_SYNTAX_KEY, i as zodV3ToOpenAPI, l as PARENT_ADDITIONAL_PROPERTIES_KEY, m as UNWRAP_ROOT_KEY, n as isZodDto, o as fixAllRefs, p as SELF_REQUIRED_KEY, r as ioSymbol, s as DEFS_KEY, t as createZodDto, u as PARENT_HAS_REFS_KEY } from "./dto-CHeB-l1i.mjs";
...
function createZodGuard({ createValidationException } = {}) {
	...
	ZodGuard = __decorate([Injectable(), __decorateMetadata("design:paramtypes", [Object, Object])], ZodGuard);
	return ZodGuard;
}
/**
...
function registerSchema(schemaRegistry, schemaName, schema) {
	if (schemaRegistry[schemaName] && !isDeepStrictEqual(schemaRegistry[schemaName], schema)) throw new Error(`[cleanupOpenApiDoc] Found multiple schemas with name \`${schemaName}\`.  Please review your schemas to ensure that you are not using the same schema name for different schemas`);
...
function cleanupSchema({ oldSchemaName, oldOpenapiSchema, version }) {
	...
	if (usesThreePointOneSyntax && version === "3.0") newOpenapiSchema = convertToOpenApi3Point0(newOpenapiSchema);
	...
	if (additionalNewSchemas[newSchemaName] && !isDeepStrictEqual(additionalNewSchemas[newSchemaName], newOpenapiSchema)) throw new Error(`[cleanupOpenApiDoc] Found multiple schemas with name \`${newSchemaName}\`.  Please review your schemas to ensure that you are not using the same schema name for different schemas`);
	return {
...
function fixParameter(parameterInput, version) {
	...
	if (PARENT_METADATA_KEY in parameter) delete parameter[PARENT_METADATA_KEY];
	if (PARENT_ADDITIONAL_PROPERTIES_KEY in parameter) delete parameter[PARENT_ADDITIONAL_PROPERTIES_KEY];
	...
	if (PARENT_ADDITIONAL_PROPERTIES_KEY in parameter) delete parameter[PARENT_ADDITIONAL_PROPERTIES_KEY];
	if (version === "3.0" && "schema" in parameter && parameter.schema && usesThreePointOneSyntax) parameter.schema = convertToOpenApi3Point0(parameter.schema);
	return parameter;
}
