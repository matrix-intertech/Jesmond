...
export default function BusinessSettings() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (!token) return;
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/v1/settings/provider/business`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const status = await handleApiError(res, () => { clearAuth(); router.replace('/login'); });
        if (status === 'ok') {
          const json = await res.json();
          setData({ name: json.name || '', abn: json.abn || '', timezone: json.timezone || '' });
        } else {
